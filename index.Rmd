...
p <-
  ...
  rename("Doméstico" = `DOMÉSTICA`, Internacional = INTERNACIONAL) %>%
  mutate(Total = `Doméstico` + `Internacional`) %>%
...
### Em quanto cada empresa participa?

#### Mercado total (doméstico e internacional)
...
### Qual o tamanho do mercado de cargas brasileiro?

```{r}
p <-
  data %>%
  group_by(`Mês/Ano`, NATUREZA) %>%
  summarise(Demanda = sum(`CARGA PAGA KM`, na.rm = TRUE) / 1e9) %>%
  pivot_wider(names_from = NATUREZA, values_from = Demanda) %>%
  rename("Doméstico" = `DOMÉSTICA`, Internacional = INTERNACIONAL) %>%
  mutate(Total = `Doméstico` + `Internacional`) %>%
  pivot_longer(cols = c(2, 3, 4),
  ...
  ggplot(aes(x = `Mês/Ano`, y = Demanda, color = Mercado)) +
  geom_line() +
  ylab("Demanda (bilhões de toneladas-km)")
...
p <-
  ...
  ggplot(aes(x =`Mês/Ano`, y = Demanda, color = Empresa)) + 
  geom_line() +
  ylab("Demanda (milhões de toneladas-km)")
