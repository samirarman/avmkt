...
data[["Mês/Ano"]] <- as.yearmon(paste0(data[["ANO"]], "-", data[["MÊS"]]))
...
```{r}
  p <-
  data %>% 
  filter(NATUREZA == "DOMÉSTICA" & `GRUPO DE VOO` %in% c("REGULAR", "NÃO-REGULAR")) %>%
...
p <-
  data %>% 
  filter(NATUREZA == "INTERNACIONAL" & `GRUPO DE VOO` %in% c("REGULAR", "NÃO-REGULAR")) %>%
  group_by(`Mês/Ano`, `EMPRESA (SIGLA)`) %>% 
  summarise(Demanda = sum(RPK, na.rm = TRUE) / 1e9) %>% 
  filter(Demanda > 0) %>%
  rename("Empresa" = `EMPRESA (SIGLA)`) %>% 
